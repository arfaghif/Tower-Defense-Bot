    ...
    public String run() {
        ...
        if (index!=-999){
            ...
            if (allenergy<8){
                ...
                List<Integer> indexList = new java.util.ArrayList<Integer>(10);
                for (int i = 0 ; i< gameState.getGameDetails().mapHeight; i++){
                    indexList.add(i);
                }
                ...
                for (int i = gameState.getGameDetails().mapHeight-1; i >=0 ; i--) {
                    ...
                    int randomInteger = (random.nextInt(i+1));
                    int y = indexList.get(randomInteger);
                    int enemyAttackOnRow = getAllBuildingsForPlayeronRowy(PlayerType.B, b -> b.buildingType == BuildingType.ATTACK, y).size();
                    int myEnergyOnRow = getAllBuildingsForPlayeronRowy(PlayerType.A, b -> b.buildingType == BuildingType.ENERGY, y).size();
                    int myDefenseOnRow = getAllBuildingsForPlayeronRowy(PlayerType.A, b -> b.buildingType == BuildingType.DEFENSE, y).size();
                    int myAttackOnRow = getAllBuildingsForPlayeronRowy(PlayerType.A, b -> b.buildingType == BuildingType.ATTACK, y).size();
                    ...
                    if (canAffordBuilding(BuildingType.ENERGY)&&((enemyAttackOnRow == 0 && myEnergyOnRow == 0) ||((myDefenseOnRow+myAttackOnRow >= 2*enemyAttackOnRow)&& myDefenseOnRow+myAttackOnRow >2 && myEnergyOnRow <2))){
                        if(getXfromPlace(y,true)<Column){
                            Column = getXfromPlace(y,true);
                            index = y;
                        }
                    }
                    indexList.remove(randomInteger);
                }
            }
        }
        ...
        if(index != -999){
            ...
            for (int i = gameState.getGameDetails().mapHeight-1; i >= 0; i--) {
                ...
                int enemyAttackOnRow = getAllBuildingsForPlayeronRowy(PlayerType.B, b -> b.buildingType == BuildingType.ATTACK, y).size();
                int myAttackOnRow = getAllBuildingsForPlayeronRowy(PlayerType.A, b -> b.buildingType == BuildingType.ATTACK, y).size();
                int DefenseOnRow = getAllBuildingsForPlayeronRowy(PlayerType.B, b -> b.buildingType == BuildingType.DEFENSE, y).size();
                //int myDefenseOnRow = getAllBuildingsForPlayeronRowy(PlayerType.A, b -> b.buildingType == BuildingType.DEFENSE, y).size();
